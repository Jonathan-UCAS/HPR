...
# from Dataloader.modelnet_C_dataloader import get_sets
...
def get_arg():
    ...
    cfg.add_argument('--data_aug',
                     default=True)  # hyperprotonet  gamma=0.8,lr=0.001,c=0.01/0.005,1shot 用c=0.01,5shot用 c=0.005
    cfg.add_argument('--epochs', default=100)  # wacv gamma=0.5, lr=0.0008,  modelnet曲率是c=0.005，
    cfg.add_argument('--decay_ep', default=5)  # scanobjectNN 实验均用c=0.01, modelnet 实验  1shot用c=0.01更好
    cfg.add_argument('--gamma', default=0.8)  # this paper 0.8
    cfg.add_argument('--lr', default=0.001)  # this paper 0.001
    cfg.add_argument('--train', action='store_true', default=True)
    cfg.add_argument('--seed', default=0)
    cfg.add_argument('--dataset', default='ScanObjectNN')  # modelnet40,modelnet40_C,ScanObjectNN,ScanObjectNN_PB
    cfg.add_argument('--data_path', default='/Dataset/ScanObjectNN/ScanObjectNN')
    # ModelNet_hdf5,ScanobjectNN_hdf5,ScanobjectNN_hdf5_PB
    cfg.add_argument('--device', default='cuda')
    cfg.add_argument('--fs_head', default='wrapped')
    # options: protonet, CIA, crossAtt_mixmodel,MetaOpt,protonet_top,hyperproto, wrapped, ori_protonet, relationnet
    cfg.add_argument('--backbone', default='hyper_dgcnn')
    # options:dgcnn,view,dgcnn_normal,ori_dgcnn,hyper_dgcnn, pointAndview
    cfg.add_argument('--exp', default='Debug')  # exp
    cfg.add_argument('--exp_description', default='')  # MetaOpt
    cfg.add_argument('--val_epoch_size', type=int, default=1)  # 默认700`
    # ======== few shot cfg =============#
...
def main(cfg):
    ...
    train_loader, val_loader = get_sets(
        data_path=cfg.data_path, fold=cfg.fold, k_way=cfg.k_way, n_shot=cfg.n_shot, query_num=cfg.query,
        data_aug=cfg.data_aug
    )
    model = fs_network(
        k_way=cfg.k_way, n_shot=cfg.n_shot, query=cfg.query, backbone=cfg.backbone, fs=cfg.fs_head
    )
    logger.debug(model)
    if cfg.train:
        train_model(model, train_loader, val_loader, cfg)
    else:
        _test_model(model, val_loader, cfg)
...
def train_model(model, train_loader, val_loader, cfg):
    ...
    lr_schedule = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=np.arange(5, cfg.epochs, cfg.decay_ep), gamma=cfg.gamma)
...
def _test_model(model, val_loader, cfg):
    global logger
    logger = get_logger()
    fsl_config = str(cfg.k_way) + "way_" + str(cfg.n_shot) + "shot_" + str(cfg.query) + "query_" + cfg.exp

    ckpt_path = os.path.join('Exp_CrossVal', cfg.fs_head, cfg.dataset + "_fold_{}".format(str(cfg.fold)), fsl_config,
                             'pth_file', 'best.pth')
...
def run_one_epoch(model, bar, mode, loss_func, optimizer=None, show_interval=10):
    ...
    for i, (x_cpu, y_cpu) in enumerate(bar):
        ...
        # label[(y[:-1]==y[-1])[:, 0]] = 1
        # print("x为：",x.shape) 105*3*1024
        num = x.shape[0]
        # print("num:",num)   5+5*20=105
        y = torch.reshape(y, (num,))

        if mode == 'train':
            optimizer.zero_grad()
            if cfg.fs_head in ['protonet', 'CIA', 'crossAtt_mixmodel', 'protonet_top', 'hyperproto', 'ori_protonet',
                               'mixmodel', 'relationnet', 'wrapped']:
