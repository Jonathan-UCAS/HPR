...
def load_scanobjectnn_data(partition):
    ...
    h5_name = DATA_DIR + '/ScanObjectNN/main_split/' + partition + '_objectdataset_augmentedrot_scale75.h5'
...
if __name__ == '__main__':
    ...
    target_path = '/home/chenxia/hyperprotoNet/dataset/ScanobjectNN_hdf5_PB'
    train_data, train_label = load_scanobjectnn_data('training')
    test_data, test_label = load_scanobjectnn_data('test')
    all_data = np.concatenate([train_data, test_data], axis=0)  # (15000, 2048, 3)
    all_label = np.concatenate([train_label, test_label], axis=0)  # (15000, 1)  (14298)
    class_list = np.arange(15)

    for c in class_list:
        list_of_points = []
        for i in range(len(all_label)):
            if all_label[i] == c:
                list_of_points.append(all_data[i])
        save_path = os.path.join(target_path, str(c), '%d.npy' % (c))
