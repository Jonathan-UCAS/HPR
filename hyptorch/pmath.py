...
def _tensor_dot(x, y):
    ...
    return res
def _tensordot(x, y):
    res = torch.einsum("bij,bkj->bik", (x, y))
    return res
...
def _mobius_addition_batch(x, y, c):
    ...
    num = num.unsqueeze(2) * x.unsqueeze(1) # 分子左项  B x C x D

    num = num + (1 - c * x2).unsqueeze(2) * y  # B x C x D 450*30*1600  分子
    denom_part1 = 1 + 2 * c * xy  # B x C
    denom_part2 = c ** 2 * x2 * y2.permute(1, 0)
    denom = denom_part1 + denom_part2  # 分母 B x C
    res = num / (denom.unsqueeze(2) + 1e-5) # B x C x D
    return res

def _mobius_addition_batch_dist(x,y,c): # B x 1024 x 3
    inner = torch.matmul(x, x.transpose(2, 1))  # B*1024*1024
    # xy = _tensordot(x, y)  # B*1024*1024
    x2 = x.pow(2).sum(-1, keepdim=True)  # B x 1024 x 1
    num = 1 + 2 * c * inner + c * x2.permute(0, 2, 1)  # B x 1024 x 1024
    num = num.unsqueeze(3) * x.unsqueeze(2) # B x 1024 x 1024 x 3
    num = num + (1 - c * x2).unsqueeze(3) * x.unsqueeze(1)  # B x 1024 x 1024 x 3
    denom_part1 = 1 + 2 * c * inner  # B x 1024 x 1024
    denom_part2 = c ** 2 * x2 * x2.permute(0, 2, 1) # B x 1024 x 1024
    denom = denom_part1 + denom_part2
    res = num / (denom.unsqueeze(3)+ 1e-5)
    return res
...
def poincare_mean(x, dim=0, c=1.0):
    ...
    up = torch.sum(lamb * x, dim=dim, keepdim=True)
    down = torch.sum(lamb, dim=dim, keepdim=True)
    mean = up / down
    mean = k2p(mean, c)
...
def _dist_matrix_knn(x, y, c):
    # sqrt_c = c ** 0.5
    return torch.norm(_mobius_addition_batch_dist(-x, y, c=c), dim=-1)

def dist_matrix_knn(x, y, c=1.0):
    c = torch.as_tensor(c).type_as(x)
    return _dist_matrix_knn(x, y, c)
...
def ptransp0_p(x, u, c):  # u为要运输的向量，从0点切空间运输到x点处切空间
