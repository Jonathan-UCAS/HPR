...
class crossAtt_mixmodel(nn.Module):
    def __init__(self, n_way, k_shot, query, c=0.005, train_c=False, train_x=False, temperature=1):
        super().__init__()
        self.n_way = n_way
        self.k_shot = k_shot
        self.query = query
...
class trip(nn.Module):
    ...
    def getout(self, support, queries):  # (63,5,256) (63,50,256)

        # support = support.permute(1, 0, 2).reshape(5, -1)  # 5*(63*256)
        # queries = queries.permute(1, 0, 2).reshape(50, -1)  # 50*(63*256)
        #
        # support = self.e2p(support)
        # queries = self.e2p(queries)
        # distance = dist_matrix(queries, support, c=self.e2p.c) / self.temperature # 50*5

        dist = torch.cdist(queries, support)  # (63,50,5)
        dist = torch.mean(dist, 0)  # (50,5)
        y_pred = (-dist).softmax(1)
        return y_pred
