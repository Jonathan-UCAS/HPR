...
def MetaOptNetHead_SVM_CS(query, support, support_labels, query_labels, n_way, n_shot, C_reg=0.1,
    ...
    block_kernel_matrix += 10.0 * torch.eye(n_way * n_support).expand(tasks_per_batch, n_way * n_support,
...
class MetaOpt(nn.Module):
    ...
    def forward(self, query, support, support_labels, query_labels, n_way, n_shot, **kwargs):
        if self.enable_scale:
            logit_query = self.scale * self.head(query, support, support_labels, query_labels, n_way, n_shot, **kwargs)
            pred, loss = self.get_out(logit_query, query_labels, n_way)
        else:
            logit_query = self.head(query, support, support_labels, n_way, n_shot, **kwargs)
            pred, loss = self.get_out(logit_query, query_labels, n_way)
        return pred, loss
