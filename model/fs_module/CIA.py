...
class CIA(nn.Module):
    ...
    def forward(self, sample_inpt, label):
        support, query = sample_inpt[:self.n * self.k], sample_inpt[self.n * self.k:]
        proto = support.reshape(self.k, self.n, -1).mean(1)
        sample_inpt = torch.cat([proto, query])
...
if __name__ == '__main__':
    ...
    sample_inpt = torch.randn((30, 256))
